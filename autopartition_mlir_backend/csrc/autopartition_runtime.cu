                                                                                   ...
                                                                                   int                 k)
{
    ...
    Tensor gCoord_full = make_identity_tensor(make_shape(m, n));
    ...
    Tensor gCoord = local_tile(gCoord_full, make_tile(bM{}, bN{}), make_coord(blockIdx.x, blockIdx.y));
...
#pragma unroll 1
    ...
    Tensor tSR_coord      = s2r_thr_copy_C.partition_D(gCoord);
    ...
    cutlass::NumericConverter<cutlass::half_t, float> round_to_fp16;
    CUTE_UNROLL
    for (int i = 0; i < size(tSR_rAcc); ++i) {
        auto coord = tSR_coord(i);
        int col = int(cute::get<1>(coord));
        float gemm_value = static_cast<float>(round_to_fp16(static_cast<float>(tSR_rAcc(i))));
        float value = static_cast<float>(round_to_fp16(gemm_value + static_cast<float>(ptr_bias[col])));
        value = 0.5f * value * (1.0f + erff(value * 0.7071067811865476f));
        tSR_rD(i) = convert(value);
    }
...
torch::Tensor autopartition_gemm_bias_gelu(torch::Tensor A, torch::Tensor B, torch::Tensor bias)
{
    ...
    auto *ptr_C = reinterpret_cast<InputElement *>(C.data_ptr<at::Half>());
...
torch::Tensor fused_gemm_bias_gelu(torch::Tensor A, torch::Tensor B, torch::Tensor bias)
{
    check_gemm_inputs(A, B);
    check_bias(B, bias);
    return autopartition_gemm_bias_gelu(A, B, bias);
}
