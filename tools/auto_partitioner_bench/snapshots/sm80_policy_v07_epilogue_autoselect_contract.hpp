...
template <int N> struct Sm80StaticLog2
{
    ...
    static_assert((N & (N - 1)) == 0, "Sm80StaticLog2 requires power-of-two N.");
    static constexpr int value = 1 + Sm80StaticLog2<(N >> 1)>::value;
};

template <> struct Sm80StaticLog2<1>
{
    static constexpr int value = 0;
};

template <int N, bool Valid> struct Sm80Log2OrZero
{
    static constexpr int value = 0;
};

template <int N> struct Sm80Log2OrZero<N, true>
{
    static constexpr int value = Sm80StaticLog2<N>::value;
};

constexpr bool sm80_epilogue_is_pow2(int value) { return value > 0 && ((value & (value - 1)) == 0); }
...
constexpr int sm80_epilogue_tiled_offset(int major,
                                         ...
                                         int swizzle_shift)
{
    int const major_tiles = logical_major_extent / row_elements;
    int const major_tile  = major / row_elements;
    int const minor_tile  = minor / minor_elements;
    int const major_local = major % row_elements;
    int const minor_local = minor % minor_elements;
    int const tile_base   = (minor_tile * major_tiles + major_tile) * row_elements * minor_elements;
                                                ...
                                                bool valid)
{
    ...
    for (int group = 0; group < sim_threads; group += issue_threads) {
        for (int lane = 0; lane < vector_elements; ++lane) {
            int bank_counts[32] = {};
            for (int thread = 0; thread < issue_threads; ++thread) {
                int const logical = (group + thread) * vector_elements + lane;
                int const major   = logical % logical_major_extent;
                int const minor   = (logical / logical_major_extent) % logical_minor_extent;
                int const offset  = sm80_epilogue_tiled_offset(major,
                                                              minor,
                                                              logical_major_extent,
                                                              row_elements,
                                                              minor_elements,
                                                              swizzle_bits,
                                                              swizzle_mbase,
                                                              swizzle_shift);
                int const bank    = ((offset * element_bytes) / 4) & 31;
                bank_counts[bank] += 1;
            }
            for (int bank = 0; bank < 32; ++bank) {
                if (bank_counts[bank] > 1) {
                    conflicts += bank_counts[bank] - 1;
                }
            }
        }
    }
    ...
    for (int group = 0; group < sim_threads; group += issue_threads) {
        for (int thread = 0; thread < issue_threads; ++thread) {
            ...
            int const offset  = sm80_epilogue_tiled_offset(major,
                                                          ...
                                                          swizzle_mbase,
                                                          swizzle_shift);
            if (((offset * element_bytes) % vector_bytes) != 0) {
                penalty += 1;
            }
        }
    }
    ...
    for (int group = 0; group < sim_threads; group += issue_threads) {
        for (int lane = 0; lane < vector_elements; ++lane) {
            ...
            for (int thread = 0; thread < issue_threads; ++thread) {
                ...
                int const major   = logical % logical_major_extent;
                int const minor   = (logical / logical_major_extent) % logical_minor_extent;
                int const offset  = minor * logical_major_extent + major;
                int const bank    = ((offset * element_bytes) / 4) & 31;
                bank_counts[bank] += 1;
            }
            for (int bank = 0; bank < 32; ++bank) {
...
{
    using Better = typename Sm80BetterEpilogueCandidate<Candidate0, Candidate1>::type;
    using type   = typename Sm80BestEpilogueCandidate<Better, Rest...>::type;
};
...
struct Sm80BestEpilogueCandidateForRow
{
    using type = typename Sm80BestEpilogueCandidate<
        ...
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 4, 4>,
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 8, 2>,
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 8, 3>,
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 8, 4>,
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 16, 2>,
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 16, 3>,
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 16, 4>,
        Sm80EpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor, RowBytes, 32, 2>,
...
struct Sm80BestEpilogueLayoutCandidate
{
    ...
    using Best128 = typename Sm80BestEpilogueCandidateForRow<Element, BlkM, BlkN, VectorBytes, IsMnMajor, 128>::type;
    using Best256 = typename Sm80BestEpilogueCandidateForRow<Element, BlkM, BlkN, VectorBytes, IsMnMajor, 256>::type;
    using type    = typename Sm80BestEpilogueCandidate<Best32, Best64, Best128, Best256>::type;
};
...
struct Sm80TensorOpEpilogueSmemLayoutSelector
{
    ...
    using BestCandidate = typename Sm80BestEpilogueLayoutCandidate<Element, BlkM, BlkN, VectorBytes, IsMnMajor>::type;

    static constexpr int CandidateCount = 48;
    static constexpr int RowElements    = BestCandidate::RowElements;
    static constexpr int MinorElements  = BestCandidate::MinorElements;
    static constexpr int SwizzleBytes   = RowElements * ElementBytes;
    static constexpr int SwizzleBase    = BestCandidate::SwizzleBits;
    ...
    using SwizzleAtom = cute::conditional_t<
        ...
        decltype(cute::composition(cute::Swizzle<SwizzleBase, SwizzleMBase, SwizzleShift>{},
                                   cute::Layout<cute::Shape<cute::Int<RowElements>, cute::Int<MinorElements>>,
                                                cute::Stride<cute::_1, cute::Int<RowElements>>>{})),
        decltype(cute::composition(cute::Swizzle<SwizzleBase, SwizzleMBase, SwizzleShift>{},
                                   cute::Layout<cute::Shape<cute::Int<MinorElements>, cute::Int<RowElements>>,
...
struct Sm80TensorOpRoleC
{
    ...
    // -------------------------------------------------------------------------
    //
    // 标准 SM80 TensorOp epilogue 里，shared memory 存 accumulator。
    //
    // 例如:
    //   ElementInput    = cutlass::half_t
    //   Accumulator     = float
    //   OutputElement   = cutlass::half_t
    //
    // 那么这里 shared memory 物理类型是 float，不是 half。

    static constexpr int EpilogueAlignmentElements =
        GmemVectorAlignment<EpilogueElement, EpilogueLogicalMajorExtent, 16>::value;
    static constexpr int EpilogueAlignmentBytes = EpilogueAlignmentElements * int(sizeof(EpilogueElement));
    ...
    static constexpr int EpilogueSwizzleBytes = EpilogueSwizzleSelector::SwizzleBytes;

    static constexpr int EpilogueSwizzleBase = EpilogueSwizzleSelector::SwizzleBase;

    static constexpr int EpilogueSwizzleMBase = EpilogueSwizzleSelector::SwizzleMBase;

    static constexpr int EpilogueSwizzleShift = EpilogueSwizzleSelector::SwizzleShift;

    static constexpr int EpilogueSwizzleRowElements = EpilogueSwizzleSelector::RowElements;

    static constexpr int EpilogueSwizzleMinorElements = EpilogueSwizzleSelector::MinorElements;

    static constexpr int EpilogueLayoutCandidateCount = EpilogueSwizzleSelector::CandidateCount;

    static constexpr int EpilogueBankConflictScore = EpilogueSwizzleSelector::BankConflictScore;

    static constexpr int EpilogueNaiveBankConflictScore = EpilogueSwizzleSelector::NaiveBankConflictScore;

    static constexpr int EpilogueVectorAlignmentPenalty = EpilogueSwizzleSelector::AlignmentPenalty;

    static constexpr int EpilogueLayoutSelectionScore = EpilogueSwizzleSelector::SelectionScore;

    using SmemLayoutAtom = typename EpilogueSwizzleSelector::SwizzleAtom;
    ...
    // -------------------------------------------------------------------------
    // 2. Source C load / final D store
    // -------------------------------------------------------------------------
    //
    // output-op / fusion 之后，register fragment 转成 OutputElement，
    // 最后直接写 global。
    //
    // 这一步不经过 output shared memory。

    static constexpr int OutputAlignmentElements =
        GmemTiledCopyAlignment<OutputElement, BlkM, BlkN, ThreadCount, IsMnMajor, GmemAlignmentBytes>::value;
    ...
    using SourceGmemToRegCopy = OutputGmemTiledCopy;
    ...
    using RegToGmemCopy = RegisterToGlobalCopyAtom;
