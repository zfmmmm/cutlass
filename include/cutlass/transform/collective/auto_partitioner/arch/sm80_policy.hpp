...
template <int InstructionBytes, int LogicalMajorBytes> struct Sm80EpilogueSwizzleBytes
{
    static constexpr int value =
        (InstructionBytes >= 128 && LogicalMajorBytes >= 128 && (LogicalMajorBytes % 128) == 0) ? 128
        : (InstructionBytes >= 64 && LogicalMajorBytes >= 64 && (LogicalMajorBytes % 64) == 0)  ? 64
        : (InstructionBytes >= 32 && LogicalMajorBytes >= 32 && (LogicalMajorBytes % 32) == 0)  ? 32
                                                                                                : 0;

    static_assert(value != 0,
                  "SM80 epilogue swizzle requires a 32/64/128-byte atom that divides the logical major row.");
};
...
struct Sm80TensorOpEpilogueSmemLayoutSelector
{
    ...
    static constexpr int InstructionBytes = InstructionThreads * VectorBytes; // 一组访问总共覆盖多少字节
    static constexpr int LogicalMajorBytes = LogicalMajorExtent * ElementBytes;

    static constexpr int SwizzleBytes = Sm80EpilogueSwizzleBytes<InstructionBytes, LogicalMajorBytes>::value;

    static_assert((SwizzleBytes % ElementBytes) == 0, "SM80 epilogue swizzle span must be element-addressable.");

    static constexpr int RowElements = SwizzleBytes / ElementBytes;

    static_assert((LogicalMajorExtent % RowElements) == 0,
                  "SM80 epilogue logical major extent must be covered by whole swizzle atoms.");

    static_assert((RowElements & (RowElements - 1)) == 0, "SM80 epilogue swizzle row elements must be power-of-two.");

    // CuTe Swizzle<Base, 3, 3> should match the physical row length in elements:
    //   half  128B atom -> 64 elements -> Base = 3
    //   float 128B atom -> 32 elements -> Base = 2
    //   double 128B atom -> 16 elements -> Base = 1
    static constexpr int SwizzleBase = Sm80StaticLog2<RowElements>::value - 3;

    static_assert(SwizzleBase >= 0, "SM80 epilogue Swizzle<Base,3,3> requires RowElements >= 8.");

    using SwizzleAtom =
        cute::conditional_t<IsMnMajor,
                            decltype(cute::composition(cute::Swizzle<SwizzleBase, 3, 3>{},
...
struct Sm80TensorOpRoleC
{
    ...
    using EpilogueSwizzleSelector = Sm80TensorOpEpilogueSmemLayoutSelector<EpilogueElement, EpilogueLogicalMajorExtent,
                                                                           EpilogueVectorBytes, IsMnMajor>;
    ...
    using RegToGmemCopy = OutputGmemTiledCopy;
